...
| E06-S04 Smart Start Resume Reference Bridge | E06 | P2 | Done | Paid Smart Start now converts real resume URLs into deduped `intake_reference` CJS assets, supports direct resume file upload in intake, keeps uploaded files prioritized for review, and makes the lighter evidence class explicit in the UI. |
...
| 2026-03-30 23:31:18Z | Smart Start resume bridge hardening | Converted Smart Start resume URLs into deduped `intake_reference` CJS assets, added direct intake file upload for resumes, prioritized uploaded files ahead of intake references in CJS + review selection, and made the lighter evidence scope explicit in both the resume UI and `resume_review` output. | Done | `components/IntakeFlow.tsx`, `components/CjsExecutionView.tsx`, `services/cjsApi.ts`, `api/index.js`, `types.ts`, `docs/career-concierge-os.md`, `docs/operations-runbook.md`, `docs/progress-log.md` |
