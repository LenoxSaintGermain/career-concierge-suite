...
- Gemini now waits for the actual Live socket `onopen` event before sending that opening turn, which avoids the early `WebSocket ... CLOSING or CLOSED` race seen in browser console logs.
...
- Hardened Gemini Live again after prod console review: the Smart Start opening question and context refresh path now use realtime text input rather than client-content turns, removing the `1007 invalid argument` close that was ending sessions immediately after the first prompt on `gemini-3.1-flash-live-preview`.
